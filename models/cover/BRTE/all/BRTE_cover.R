...
load("models/cover/BRTE/all/inits/inits.Rdata")# saved.state, second element is inits
...
# model
jm <- jags.model(file = "models/cover/BRTE/all/BRTE_cover_zib.jags",
                 inits = initslist,
                 n.chains = 3,
                 data = datlist)
# update(jm, 10000)
...
# saved.state[[1]]
# save(saved.state, file = "models/cover/BRTE/all/inits/inits.Rdata")

save(coda.out, file = "models/cover/BRTE/all/coda/coda.Rdata")
...
coda.rep <- coda.samples(jm, variable.names = params,
                         n.iter = 15000, thin = 5)
save(coda.rep, file = "models/cover/BRTE/all/coda/coda_rep.Rdata")
