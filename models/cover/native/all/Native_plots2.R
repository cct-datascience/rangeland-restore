...
load(file = "coda/coda2.Rdata") # coda.out
load(file = "coda/coda_rep2.Rdata") # coda.rep
...
fig_2a <- ggplot() +
  ...
  #            shape = 8) +
  geom_hline(yintercept = 0, lty = 2) +
  scale_y_continuous(expression(paste(Delta, "Native grass proportion cover"))) +
  scale_x_discrete(limits = rev(levels(betas$var)), labels = rev(beta.labs2)) +
  # scale_color_manual(values = c("goldenrod3", "forestgreen")) +
  coord_flip() +
  theme_bw(base_size = 14) +
...
jpeg(filename = "plots/fig3_betas.jpg", 
     width = 8, 
     height = 6, 
     units = "in",
     res = 600)
