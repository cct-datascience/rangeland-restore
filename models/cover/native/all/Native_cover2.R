...
# model
jm <- jags.model(file = "Native_cover_zoib2.jags",
                 inits = initslist,
                 n.chains = 3,
                 data = datlist)
# update(jm, 10000)
...
# plot chains
mcmcplot(coda.out, parms = c("deviance", "rho", "mu", "beta",
                             "alpha.star",  "eps.star", 
                             "sig", "sig.eps"))
traplot(coda.out, parms = c("sig", "sig.eps"))
...
save(coda.out, file = "coda/coda2.Rdata")
...
coda.rep <- coda.samples(jm, variable.names = params,
                         n.iter = 15000, thin = 5)
save(coda.rep, file = "coda/coda_rep2.Rdata")
